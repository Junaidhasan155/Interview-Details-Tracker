...
export function SmartResourceCard({ resource, onUpdate }: SmartResourceCardProps) {
  ...
  const getStatusColor = (status: Resource['status']) => {
    switch (status) {
      ...
      case 'in-progress': return 'bg-info/20 text-info-foreground border-info/30';
      case 'not-started': return 'bg-muted/50 text-muted-foreground border-muted';
  ...
  const getPriorityColor = (priority: Resource['priority']) => {
    switch (priority) {
      ...
      case 'low': return 'bg-success/20 text-success-foreground border-success/30';
      default: return 'bg-muted/50 text-muted-foreground border-muted';
    }
  };
